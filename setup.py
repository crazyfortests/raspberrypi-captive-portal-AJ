...
WifiSSID = os.getenv("WifiSSID", "AJ_")
WifiPasswd = os.getenv("WifiPasswd", "Attherate123")
...
def build_server():
    ...
    #----AJ----
    #Change host_name as per the environmental variable set at run time.
    if os.getenv('Host_name'):
        print('changing hosname',os.getenv('Host_name'))
        with open('./server/build/server.js','r')as f:
            _ = f.read().replace('aj.com.in',os.getenv('Host_name'))
        with open('./server/build/server.js', 'w') as f:
            f.write(_)
    #----AJ----
...
def done():
    ...
    final_msg = f"""Awesome, we are done here. Grab your phone and look for the
    "{WifiSSID} AP" WiFi (password: "{WifiPasswd}").
