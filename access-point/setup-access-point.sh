...
#---------AJ---------
cp ./access-point/hostapd.conf /tmp/hostapd.conf
if [ -n "$WifiSSID" ] && [ -n "$WifiPasswd" ]; then
    sed -i "s/^ssid=.*/ssid=$WifiSSID/" /tmp/hostapd.conf
    sed -i "s/^wpa_passphrase=.*/wpa_passphrase=$WifiPasswd/" /tmp/hostapd.conf
fi
